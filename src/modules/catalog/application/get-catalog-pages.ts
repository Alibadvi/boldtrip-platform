...
type CountryRecord = Country & { sortOrder?: number }
...
async function findCountry(slug: string): Promise<Country | null> {
  ...
  const record = result.docs[0] as unknown as CountryRecord | undefined
...
export const getCountries = cache(async (): Promise<Country[]> => {
  ...
  return (result.docs as unknown as CountryRecord[]).map(countryFromRecord)
})

export const getFeaturedCountries = cache(async (): Promise<Country[]> => {
  const payload = await getPayload({ config: configPromise })
  const result = await payload.find({
    collection: 'countries',
    depth: 0,
    ...
    sort: 'sortOrder',
    where: {
      and: [{ featuredOnHomepage: { equals: true } }, publishedOnly],
    },
  })
...
export const getCountryPage = cache(async (slug: string): Promise<CountryPageData | null> => {
  ...
  const result = await payload.find({
    ...
    sort: 'sortOrder',
    where: {
      and: [{ country: { equals: country.id } }, publishedOnly],
    },
  })

  return {
    country,
    visas: (result.docs as unknown as VisaRecord[]).map(visaSummaryFromRecord),
  }
})

export const getVisaPage = cache(
  async (countrySlug: string, visaSlug: string): Promise<VisaPageData | null> => {
    ...
    const result = await payload.find({
      ...
      pagination: false,
      where: {
        and: [{ country: { equals: country.id } }, { slug: { equals: visaSlug } }, publishedOnly],
      },
    })
    const record = result.docs[0] as unknown as VisaRecord | undefined

    return record ? { country, visa: visaDetailFromRecord(record) } : null
  },
)

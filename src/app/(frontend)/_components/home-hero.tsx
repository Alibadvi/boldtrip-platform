...
export function HomeHero({ content }: HomeHeroProps) {
  return (
    <section
      ...
      className="relative isolate -mt-24 overflow-hidden bg-brand-950 pt-36 text-white sm:pt-40 lg:min-h-[900px] lg:pt-44"
    >
      {/* Main background */}
      <div
        aria-hidden="true"
      ...
      <Container className="relative grid items-center gap-6 pb-12 sm:pb-16 lg:min-h-[610px] lg:grid-cols-[0.92fr_1.08fr] lg:gap-4 lg:pb-10">
        {/* Hero content */}
        <div className="relative z-20 max-w-3xl motion-safe:animate-page-enter">
          <h1
            id="homepage-hero-title"
            className="text-[clamp(2.65rem,6vw,5.6rem)] font-black leading-[1.3] tracking-[-0.04em] text-white"
          >
          ...
          <p className="mt-6 max-w-2xl text-base leading-9 text-white/68 sm:mt-7 sm:text-lg sm:leading-10">
            {content.description}
          </p>
          ...
          {content.highlights.length > 0 && (
            <div className="mt-9 grid max-w-2xl gap-3 border-y border-white/10 py-5 sm:grid-cols-3 sm:gap-5">
        ...
        {/* Hero artwork */}
        <div className="relative mx-auto h-[390px] w-full max-w-[720px] sm:h-[520px] lg:h-[650px] lg:max-w-none">
          {/* Artwork spotlight */}
          <div
            aria-hidden="true"
        ...
        <div className="grid border-y border-white/12 md:grid-cols-3">
          {quickActions.map((item, index) => (
            <Link
              key={item.href}
              href={item.href}
              className={`group relative flex min-h-24 items-center gap-4 px-2 py-5 transition-colors duration-300 hover:bg-white/5 sm:px-5 ${
                index < quickActions.length - 1
                  ? 'border-b border-white/10 md:border-b-0 md:border-l'
                  : ''
              }`}
            >
              <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-white/8 text-accent-300 transition-[transform,background-color] duration-300 group-hover:-translate-y-1 group-hover:bg-white/13">
                <TravelIcon
                  name={item.icon}
                  className="size-6"
                />
              ...
              <span
                dir="ltr"
                className="mr-auto text-xs font-bold tracking-[0.18em] text-white/25 transition-colors duration-300 group-hover:text-accent-300"
              >
                {item.number}
              </span>
              ...
              <TravelIcon
                name="arrow"
                className="size-5 shrink-0 text-white/35 transition-[transform,color] duration-300 group-hover:-translate-x-1 group-hover:text-accent-300"
              />
            </Link>
          ))}
        </div>
